...
library(stringr)
library(plyr)
new.aco <- anti_join(aco.part,tmp, by = "Participant.Legal.Business.Name")
...
test4 <- test2 %>% 
	filter(short_name_count>4)

test4[which(test4$n ==2),]

test3 <- test2 %>% 
	filter(n==1) %>% 
	select(short_name)

part.new.aco <- semi_join(new.aco,test3)

test <- str_extract_all(df.nppes$new.name3, "\\b(\\w{4})")
df.nppes$short_name <- sapply(test, paste, collapse = "")

nppes.aco.short.name <- right_join(df.nppes, part.new.aco, by = "short_name")
tmp5 <- nppes.aco.short.name %>%  
	filter(!is.na(provider_organization_name_legal_business_name)) %>% 
...
nppes.aco.short.name <- semi_join(nppes.aco.short.name, tmp6)
```
...
length(unique(test3$Participant.Legal.Business.Name)) #11929
```
### Try to use the same method on organizational NPIs. Or try to get the first four letters of any words longer than 3 letters.

```{r}
tmp5 <- anti_join(part.new.aco, nppes.aco.short.name, by = "Participant.Legal.Business.Name")
